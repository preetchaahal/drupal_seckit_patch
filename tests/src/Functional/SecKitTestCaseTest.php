...
class SecKitTestCaseTest extends BrowserTestBase {
   ...
   * @var array
   */
  protected static $modules = ['seckit', 'filter', 'seckit_test'];
  ...
  public function testCspHasAllDirectives() {
    ...
    $this->drupalGet('admin/config/system/seckit');
    $this->submitForm($form, t('Save configuration'));
    $script = $this->assertSession()->elementExists('css', '#testScript');
    $nonce = $script->getAttribute('nonce');
    $this->assertNotEmpty($nonce);
    $expected = sprintf("default-src *; script-src 'self' 'nonce-%s'; object-src *; style-src *; img-src *; media-src *; frame-src *; frame-ancestors *; child-src *; font-src *; connect-src *; report-uri %s; upgrade-insecure-requests", $nonce, base_path() . $this->reportPath);
